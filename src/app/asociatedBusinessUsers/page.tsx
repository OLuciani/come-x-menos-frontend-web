    ...
    <div>
      ...
      <div className="bg-white border-2 shadow-lg rounded-lg p-4 lg:py-4 h-screen">
        ...
        {totalUsers && totalUsers > 0 ? (
          ...
          <p className="text-lg text-center mt-[20%] md:mt-[10%] font-bold ">Esperando datos</p>
        )}
      </div>
...
const AsociatedBusinessUsers = () => {
  ...
  const [businessUsers, setBusinessUsers] = useState<ActiveUser[]>([]);
  const [totalUsers, setTotalUsers] = useState<number>(0);
  const [totalActiveBusinessUsers, setTotalActiveBusinessUsers] = useState<number>(0);
  const [totalInactiveBusinessUsers, setTotalInactiveBusinessUsers] = useState<number>(0);
  const [loading, setLoading] = useState<boolean>(true);
  ...
  useEffect(() => {
    const fetchBusinessUsers = async () => {
      try {
        ...
        const activeUsers = response.filter((user: ActiveUser) => user.status === "active");
        setTotalActiveBusinessUsers(activeUsers.length);

        const inactiveUsers = response.filter((user: ActiveUser) => user.status === "pending");
        setTotalInactiveBusinessUsers(inactiveUsers.length);
      } catch (error) {
    ...
    <div>
      ...
      {!selectedUser ? (
        <div className="bg-white border-2 shadow-lg rounded-lg p-4 lg:py-4 h-screen">
          ...
          <div className="bg-[#FFCF91] rounded-t-lg mb-5 pb-2">
            <h2 className="text-xl lg:text-2xl font-semibold text-[#2C2C2C] text-center py-2 px-2">
              Usuarios asociados a mi cuenta
            </h2>
            <div className="px-4 custom-w-450:px-8 md:px-48 lg:px-44 xl:px-56 flex justify-between">
              <p className="text-base lg:text-lg font-semibold text-[#2C2C2C]">
                Activos: {totalActiveBusinessUsers}
              </p>
              <p className="text-base lg:text-lg font-semibold text-[#2C2C2C]">
                Inactivos: {totalInactiveBusinessUsers}
              </p>
            </div>
          </div>
          ...
          {loading ? (
            <>
              ...
              <p className="text-base text-center mt-10 font-semibold ">Esperando datos</p>
            </>
          ) : totalUsers > 0 ? (
            <ul className="w-full flex flex-col items-center">
              {businessUsers.map((businessUser) => (
                <li
                  key={businessUser._id}
                  className="w-full md:w-[750px] md:justify-center p-2 border-[3px] border-gray-400 hover:border-[#FFCF91] rounded-lg cursor-pointer mb-5"
                  onClick={() => setSelectedUser(businessUser)}
                >
                  <div className="ml-3">
                    <strong className="text-xl">
                    ...
                    <p>
                      <span className="font-semibold">Email:</span> {businessUser.email}
                    </p>
                    <p>
                      <span className="font-semibold">Estado de la cuenta de usuario:</span>{" "}
                      {businessUser.status === "active"
                        ? "Activa"
                        : businessUser.status === "pending"
            ...
            <p className="text-lg text-center mt-[20%] md:mt-[10%] font-bold">
              No hay usuarios asociados.
            </p>
          )}
        </div>
      ) : (
        <AsociatedBusinessUserDetail user={selectedUser} onClose={() => setSelectedUser(null)} />
      )}
    </div>
  );
};
