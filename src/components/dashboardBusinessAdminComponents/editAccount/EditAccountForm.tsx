  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //Si el token expiró va a mostrar un modal informando al usuario
        if (fetchedBusiness === "TOKEN_EXPIRED") {
          setIsModalOpen(true); // Muestra el modal TokenExpiredModal.tsx si el token es inválido y redirecciona a login
          return; // Detiene la ejecución para evitar errores con response
        }
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (userRole === roleBusinessDirector) {
          ...
          //Si el token expiró va a mostrar un modal informando al usuario
          if (businessUpdate === "TOKEN_EXPIRED") {
            setIsModalOpen(true); // Muestra el modal TokenExpiredModal.tsx si el token es inválido y redirecciona a login
            return; // Detiene la ejecución para evitar errores con response
          }
        ...
        if (userUpdate.message !== "Usuario actualizado correctamente") {
          //Variables para el mensaje de edición exitosa de la cuenta al usuario
          const title: string =
            "No puedes editar la cuenta, no tienes credenciales para hacerlo";
          setMessageTitle(title);
        ...
        <div
          className={`${
            businessDirectorRole && userRole !== businessDirectorRole
              ? "hidden"
              : "w-full flex flex-col items-center mx-auto gap-6"
          } `}
        >
          <Input
