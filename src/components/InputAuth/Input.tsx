...
interface InputProps {
  ...
  minLength?: number;
  min?: number;
  value?: string | number | File | Blob | null; // Cambiado para aceptar 'string', 'number', 'File', 'Blob', 'null'
  accept?: string;
  onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
  onBlur?: (e: React.FocusEvent<HTMLInputElement>) => void;
  extraOnClick?: (e: React.FocusEvent<HTMLInputElement>) => void;
    ...
    <div className="flex flex-col w-full mt-3">
      ...
      {type === "file" ? (
        ...
        <input
          ...
          onChange={onChange}
          minLength={minLength}
          value={typeof value === "number" || typeof value === "string" ? value : ""} // Verificación para tipos 'number' o 'string'
          onBlur={onBlur}
        />
      )}
    </div>
